# Nodo SUBDERE — documentación

Documentación del Nodo SUBDERE: el punto único por el que un municipio entrega información a las instituciones que se la piden, y recibe constancia de lo entregado — y, en el ámbito SGM, el catálogo por el que consume módulos publicados detrás de la Plataforma de Control.

La maqueta HTML vive en [`prototipos/`](../prototipos/) y se publica en [GitHub Pages](https://jaimehernandeze.github.io/nodo-subdere/).

## Documentos

| Documento | Qué contiene |
|---|---|
| [`plataforma-control.md`](plataforma-control.md) | **Propuesta.** Plataforma Institucional de Gestión de APIs (Plataforma de Control): base técnica licitables; control, logs y casos de uso |
| [`flujo_2.md`](flujo_2.md) | Diagrama de **producción** de la Plataforma de Control (zonas de red) |
| [`flujo_1.md`](flujo_1.md) | Diagrama de **demo** de la Plataforma de Control (un servidor) |
| [`adr-2026-09-estandar-legible-por-maquina.md`](adr-2026-09-estandar-legible-por-maquina.md) | **Decisión.** El estándar de cada nodo se publica como especificación legible por máquina; el catálogo la renderiza y no la transcribe |
| [`maqueta.md`](maqueta.md) | Las páginas de la maqueta, el modelo de datos del catálogo y las preguntas para el QA |
| [`nodo-lp-precedente.md`](nodo-lp-precedente.md) | El Nodo Laboral y Previsional de la Subsecretaría de Previsión Social, en operación desde noviembre de 2025: qué se copia, qué no, y qué advertencias deja |

## Estado

Maqueta para revisión, septiembre de 2026. Nada de lo que muestra está comprometido institucionalmente.
